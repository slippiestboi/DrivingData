...
def confirm():
    ...
    if len(date) > 0 and len(location) > 0 and len(duration) > 0 and (tod1 == True) and (tod2 == False) and (tod3 == False):
        tod = "morning"
        msg = ("You drove at %s, \non %s, \nfor %s minute(s),\n at %s, in the %s!\nSuccessfully printed information to\nDrivingStatistics.txt" %(location, date, duration, time, tod))
        txtMsg = ("You drove at %s, on %s, for %s minute(s), at %s, in the %s!\r\n" %(location, date, duration,time, tod))
        respond(msg)
...
dateFrame.place(relheight=0.15, relwidth= 1, rely=0)
...
locateFrame.place(relheight=0.15, relwidth = 1, rely = POS)
...
durFrame.place(relheight=0.15, relwidth=1, rely=POS*2)
...
"""time"""
timeFrame = tk.Frame(canvas)
timeFrame.place(relheight=0.15, relwidth=1, rely=POS*3)
timeLabel = tk.Label(timeFrame, text="Enter time driven, ex:'8:00'")
timeLabel.place(relwidth=0.7, relx = 0.15, rely = 0.2)
timeEntry = tk.Entry(timeFrame, justify="center")
timeEntry.place(relwidth=0.5, relx=0.25, rely=0.7)
"""TimeofDay"""
todFrame = tk.Frame(canvas)
todFrame.place(relheight=0.15, relwidth=1, rely=POS*4 )
...
night = tk.Checkbutton(todFrame, text="evening", variable=var3, onvalue=1, offvalue=0)
night.place(relx=0.7, rely=0.7)
"""enterButton"""
buttonFrame = tk.Frame(canvas)
buttonFrame.place(relheight=0.15, relwidth=1, rely=POS*5)
